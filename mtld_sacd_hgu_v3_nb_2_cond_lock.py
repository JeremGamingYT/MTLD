# -*- coding: utf-8 -*-
"""
MTLD v3.3 - SACD-HGU + Conditioning + Identity Lock
"""
...
class Config:
    ...
    LAMBDA_LATENT_TF = 150.0
    LAMBDA_LATENT_SS = 150.0
    LAMBDA_LATENT_DELTA = 30.0
    LAMBDA_RECOVERY = 20.0
    LAMBDA_MMD = 2.0
    # Scheduled sampling
    ...
    # IO
    MODEL_SAVE_PATH = "./models_mtld_v3.3/"
    OUTPUT_SAVE_PATH = "./outputs_mtld_v3.3/"
    SAVE_EPOCH_INTERVAL = 2
    MAX_CHECKPOINTS = 3
    RESUME_TRAINING = False
    CHECKPOINT_TO_RESUME = "./models_mtld_v3.3/mtld_v3.3_checkpoint_epoch_10.pth"
    # GPU monitor
    ...
    # --- Conditioning ---
    C_RAW_DIM = 4                        # [darkness, motion, edge_density, saturation]
    C_BASE_EMB_DIM = 64                  # embedding size from raw proxies
    ANIME_EMB_DIM = 32                   # embedding size for anime_id (new)
    C_EMB_DIM = C_BASE_EMB_DIM + ANIME_EMB_DIM
    CFG_DROP_PROB = 0.2                  # p(c -> 0) during training
    CFG_STRENGTH = 2.0                   # default guidance strength at inference
    USE_PROXY_COND = True
    USE_DECODER_FILM = True
    USE_GRU_FILM = True

    # --- Identity lock ---
    USE_ANIME_ID_LOCK = True
    LAMBDA_ANIME_CE = 0.5                # weight for anime-id cross-entropy on predicted latents
    NO_TELEPORT_HINGE = True
    NO_TELEPORT_MARGIN = 1.5

    # --- Inference guard ---
    ID_GUARD_START = 32                  # start guarding after this many frames
    ID_GUARD_BETA = 0.2                  # latent anchor blend
    ID_GUARD_REENC_EVERY = 12            # every N frames, blend with re-encode
    ID_GUARD_TOL = 2                     # allow up to 2 off-id frames before correction kick
    GEN_COND_MODE = 'auto'               # 'auto' or 'manual'
    GEN_MANUAL_RAW = [1.0, 0.8, 0.6, 0.5]
    GEN_CFG_STRENGTH = 3.0

    # Will be set at runtime after scanning dataset:
    NUM_ANIME = 1


def _anime_key_from_dir(name: str):
    base = os.path.basename(name).lower()
    key = re.split(r'[_\-\.\s]', base)[0]
...
class AnimeFrameDataset(Dataset):
    def __init__(self, root_dir, sequence_length, transform=None, config: Config=None):
        ...
        keys = [_anime_key_from_dir(d) for d in arc_dirs]
        uniq = sorted(list(dict.fromkeys(keys)))
        self.anime_to_idx = {k:i for i,k in enumerate(uniq)}
        self.idx_to_anime = {i:k for k,i in self.anime_to_idx.items()}

        total_valid_sequences = 0
        for arc_dir in arc_dirs:
            arc_path = os.path.join(root_dir, arc_dir)
            image_paths = sorted(
                glob.glob(os.path.join(arc_path, "*.png")) + glob.glob(os.path.join(arc_path, "*.jpg")) + glob.glob(os.path.join(arc_path, "*.jpeg")),
            ...
            if len(image_paths) >= self.sequence_length:
                ...
                key = _anime_key_from_dir(arc_dir)
                self.arc_anime_id.append(self.anime_to_idx[key])
    ...
    def __getitem__(self, idx):
        ...
        anime_id = self.arc_anime_id[arc_index]
        return torch.stack(images), torch.tensor(anime_id, dtype=torch.long)
...
def get_dataloader(config: Config):
    ...
    config.NUM_ANIME = dataset.num_anime

    dataloader = DataLoader(
        dataset, batch_size=config.BATCH_SIZE, shuffle=True,
        num_workers=config.NUM_WORKERS, pin_memory=True, drop_last=True
    )
    return dataloader, dataset.total_sequences(), dataset
...
class FiLM1dTo2d(nn.Module):
    ...
    def forward(self, feat, c_emb):
        ...
        if feat.dim()==4 and c_emb.dim()==2:
            B,C,H,W = feat.shape
            gamma, beta = self.to_params(c_emb).chunk(2, dim=-1)
...
class CompositionalAnimeDecoder(nn.Module):
    ...
    def forward(self, z, noise_code, cond_emb=None):
        ...
        if seq:
            b,s,d = z.shape
            z_flat = z.view(b*s, d)
            n_flat = noise_code.view(b*s, -1) if noise_code.dim()==3 else noise_code
        else:
            b,d = z.shape; s = 1
            z_flat = z; n_flat = noise_code
        ...
        if self.use_film and cond_emb is not None:
            if cond_emb.dim()==2: c_flat = cond_emb
            elif cond_emb.dim()==3: c_flat = cond_emb.reshape(b*s, -1)
            else: c_flat = None
            if c_flat is not None: x = self.film_dec(x, c_flat)
    ...
    def forward(self, z_start, max_len, cond_emb=None, guidance_strength=0.0):
        if cond_emb is None or self.c_emb_dim == 0 or guidance_strength <= 0.0:
            ...
            for _ in range(max_len):
                x_in = torch.cat([x_latent, cond_emb], dim=-1).unsqueeze(1) if (cond_emb is not None and self.c_emb_dim>0) else x_latent.unsqueeze(1)
        ...
        for _ in range(max_len):
            ...
            z_c = self.head(gru_out_c)
            z_u = self.head(gru_out_u)
            z_next = z_u + s * (z_c - z_u)
            outs.append(z_next)
            x_latent_c = z_next.squeeze(1)
            x_latent_u = x_latent_c
        return torch.cat(outs, dim=1)
...
class MTLD(nn.Module):
    def __init__(self, config: Config):
        ...
        self.cond_encoder = ConditionEncoder(config.C_RAW_DIM, config.C_BASE_EMB_DIM)
        self.anime_emb = nn.Embedding(config.NUM_ANIME, config.ANIME_EMB_DIM)
        self.anime_head = nn.Sequential(nn.Linear(config.LATENT_DIM, 128),
                                        nn.ReLU(),
                                        nn.Linear(128, config.NUM_ANIME))
...
def save_checkpoint(epoch, model, opt_g, scaler, config: Config):
    ...
    filename = os.path.join(config.MODEL_SAVE_PATH, f"mtld_v3.3_checkpoint_epoch_{epoch}.pth")
    state = {
        'epoch': epoch,
        'model_state_dict': model.state_dict(),
        'optimizer_g_state_dict': opt_g.state_dict(),
        'scaler_state_dict': scaler.state_dict(),
        'num_anime': config.NUM_ANIME
    }
    try:
    ...
    try:
        ...
        files = [f for f in os.listdir(config.MODEL_SAVE_PATH) if f.startswith("mtld_v3.3_checkpoint_epoch_") and f.endswith(".pth") ]
...
def compute_proxy_condition(real_seq_imgs, device):
    with torch.no_grad():
        ...
        x01 = (real_seq_imgs + 1.0) / 2.0
        lum = 0.299*x01[:,:,0] + 0.587*x01[:,:,1] + 0.114*x01[:,:,2]
        darkness = 1.0 - lum.mean(dim=(1,2,3)); darkness = darkness.clamp(0,1)
        if S > 1:
            diff = (lum[:,1:,:,:] - lum[:,:-1,:,:]).abs().mean(dim=(1,2,3))
        else:
            diff = torch.zeros(B, device=device)
        motion = diff.clamp(0,1)
        real_flat = real_seq_imgs.reshape(B*S, C, H, W)
        edges = sobel_edges(real_flat)
        edge_density = edges.mean(dim=(1,2,3)).view(B, S).mean(dim=1).clamp(0,1)
        mean_rgb = x01.mean(dim=2)
        std_rgb = x01.std(dim=2)
        sat = (std_rgb / (mean_rgb + 1e-4)).mean(dim=(1,2,3)).clamp(0,1)
        c_raw = torch.stack([darkness, motion, edge_density, sat], dim=-1)
        return c_raw
...
def train_mtld():
    ...
    dataloader, total_sequences, dataset = get_dataloader(config)
    model = MTLD(config).to(config.DEVICE)
    ema = EMA(model, decay=0.999)

    params = list(model.encoder.parameters()) + list(model.decoder.parameters()) +              list(model.trajectory_generator.parameters()) + list(model.proj.parameters()) +              list(model.anime_emb.parameters()) + list(model.anime_head.parameters())
    ...
    print("\nStart training (SACD-HGU, conditioning + identity lock)...")
    try:
        for epoch in range(start_epoch, config.EPOCHS):
            ...
            for i, batch in enumerate(pbar):
                real_seq_imgs, anime_id = batch
                real_seq_imgs = real_seq_imgs.to(config.DEVICE); anime_id = anime_id.to(config.DEVICE)
                b, s, c, h, w = real_seq_imgs.shape
                ...
                if config.USE_PROXY_COND and model.cond_encoder is not None and model.cond_encoder.emb_dim>0:
                    c_raw = compute_proxy_condition(real_seq_imgs, config.DEVICE)
                    c_emb = model.cond_encoder(c_raw)
                else:
                    c_emb = None
                a_emb = model.anime_emb(anime_id)
                if c_emb is None: cond_emb = torch.cat([torch.zeros_like(a_emb), a_emb], dim=-1)
                else: cond_emb = torch.cat([c_emb, a_emb], dim=-1)

                if config.CFG_DROP_PROB > 0:
                    drop_mask = (torch.rand(b, 1, device=config.DEVICE) < config.CFG_DROP_PROB).float()
                    cond_emb = (1.0 - drop_mask) * cond_emb
                ...
                with torch.amp.autocast(device_type="cuda", dtype=torch.float16, enabled=(config.DEVICE=="cuda")):
                    ...
                    z_future_pred_tf = model.trajectory_generator.rollout_teacher(z_start_true, z_future_true, cond_emb=cond_emb)
                    z_future_pred_ss = model.trajectory_generator.rollout_scheduled(
                        z_start_true, z_future_true, sched_prob=sched_p, noise_std=config.NOISE_INJ_STD, cond_emb=cond_emb
                    )
                    z_future_pred_rec = model.trajectory_generator.rollout_scheduled(
                        z_start_true, z_future_true, sched_prob=max(0.5, sched_p), noise_std=config.RECOVERY_NOISE_STD, cond_emb=cond_emb
                    )

                    z_pred_full_seq = torch.cat([z_start_true.unsqueeze(1), z_future_pred_ss], dim=1)
                    noise_code = sample_noise_code((b, s, config.NOISE_EMB_DIM), config.DEVICE)

                    cond_seq = cond_emb.unsqueeze(1).expand(b, s, -1)
                    ...
                    logits = model.anime_head(z_pred_full_seq.reshape(-1, z_pred_full_seq.size(-1)))
                    labels = anime_id.view(b,1).repeat(1, s).reshape(-1)
                    loss_anime_ce = F.cross_entropy(logits, labels)
            ...
            if (epoch + 1) % config.SAVE_EPOCH_INTERVAL == 0:
                ...
                with torch.no_grad():
                    ...
                    try:
                        ...
                        cond_seq_eval = cond_seq if 'cond_seq' in locals() else None
                        with torch.amp.autocast(device_type="cuda", dtype=torch.float16, enabled=(config.DEVICE=="cuda")):
                            eval_imgs, *_ = model.decoder(z_pred_full_seq, noise_eval, cond_emb=cond_seq_eval)
...
def compute_proxy_condition_from_priming(priming_tensor, device):
    x = priming_tensor.unsqueeze(1)
    return compute_proxy_condition(x, device)


def _majority_id(id_seq):
    cnt = Counter(id_seq)
...
def generate_sequence(model_path, priming_image_path, num_frames_to_generate, config: Config,
    ...
    print("--- Generating sequence (v3.3 + Identity Guard) ---")
    ...
    if isinstance(checkpoint, dict) and 'model_state_dict' in checkpoint:
        if 'num_anime' in checkpoint:
            config.NUM_ANIME = checkpoint['num_anime']
    ...
    if manual_cond_raw is not None:
        c_raw = build_manual_condition(manual_cond_raw, config.DEVICE)
    else:
        c_raw = compute_proxy_condition_from_priming(priming_tensor, config.DEVICE)
    c_emb = MTLD(config).cond_encoder(c_raw) if (Config.C_BASE_EMB_DIM>0) else None  # quick stateless encoder

    with torch.no_grad():
        z_start = model.encoder(priming_tensor)
        init_logit = model.anime_head(z_start)
        target_id = int(init_logit.argmax(dim=-1).item()) if init_logit is not None else 0

        a_emb = model.anime_emb(torch.tensor([target_id], device=config.DEVICE))
        if c_emb is None:
            cond_emb = torch.cat([torch.zeros_like(a_emb), a_emb], dim=-1)
        else:
            cond_emb = torch.cat([c_emb.to(config.DEVICE), a_emb], dim=-1)
        ...
        z_future = model.trajectory_generator.forward(z_start, max_len=num_frames_to_generate - 1,
                                                      cond_emb=cond_emb, guidance_strength=guidance_strength)
        z_full_seq = torch.cat([z_start.unsqueeze(1), z_future], dim=1)

        with torch.no_grad():
            logits = model.anime_head(z_full_seq.reshape(-1, z_full_seq.size(-1)))
            ids = logits.argmax(dim=-1).view(1, -1)[0].tolist()
            anchor_id = _majority_id(ids[:max(config.ID_GUARD_START, 8)])
            beta = config.ID_GUARD_BETA
            for t in range(1, num_frames_to_generate):
                if t < config.ID_GUARD_START:
                    continue
                if ids[t] != anchor_id:
                    z_full_seq[0, t, :] = (1-beta)*z_full_seq[0, t, :] + beta*0.5*(z_full_seq[0, t-1, :] + z_full_seq[0, 1, :])

        noise_code = sample_noise_code((1, num_frames_to_generate, config.NOISE_EMB_DIM), config.DEVICE)
        cond_seq = cond_emb.unsqueeze(1).expand(1, num_frames_to_generate, -1)
        gen_imgs_1, *_ = model.decoder(z_full_seq, noise_code, cond_emb=cond_seq)

        if config.ID_GUARD_REENC_EVERY > 0:
            with torch.no_grad():
                z_reenc = model.encoder(gen_imgs_1.squeeze(0))
                for t in range(num_frames_to_generate):
                    if t % config.ID_GUARD_REENC_EVERY == 0 and t>0:
                        z_full_seq[0, t, :] = (1-config.ID_GUARD_BETA)*z_full_seq[0, t, :] + config.ID_GUARD_BETA*z_reenc[t,:]
            gen_imgs, *_ = model.decoder(z_full_seq, noise_code, cond_emb=cond_seq)
        else:
            gen_imgs = gen_imgs_1

    generated_imgs_seq = gen_imgs.squeeze(0)
    ...
    for i, img_tensor in enumerate(generated_imgs_seq):
        ...
        out_frame_path = os.path.join(output_dir_frames, f"frame_{i:04d}.png")
        img_pil.save(out_frame_path)
...
if __name__ == '__main__':
    ...
    MODEL_PATH = os.path.join("best-v3.3/mtld_v3.3_checkpoint_epoch_24.pth")
    PRIMING_PATH = "animes_dataset/beastars_ending/frame_0001.png"
    FRAMES = 250
    ...
    if MODE == 'train':
        _, _, _ = get_dataloader(cfg)  # sets cfg.NUM_ANIME
        train_mtld()
    else:
