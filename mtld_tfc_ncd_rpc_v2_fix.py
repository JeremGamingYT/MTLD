...
def ranked_patch_contrast_loss(projector: PerceptualProjector, real_imgs, fake_imgs, margin: float, patch_size: int):
    """
    Ranked Patch Contrast avec découpe de patches batched via F.unfold (évite l'indexation avancée illégale).
    ...
    ps = max(ps, 2)  # garde une taille minimale

    def pick_unfold_patches(x):
        # x: (B,C,H,W)
        B,C,H,W = x.shape
        # Extraire tous les patches ps x ps avec stride 1
        patches = F.unfold(x, kernel_size=ps, stride=1)  # (B, C*ps*ps, L)
        L = patches.size(-1)
        # index aléatoire par échantillon
        idx = torch.randint(0, L, (B,), device=x.device)
        idx_expand = idx.view(B, 1, 1).expand(B, C*ps*ps, 1)
        sel = torch.gather(patches, 2, idx_expand)       # (B, C*ps*ps, 1)
        sel = sel.view(B, C, ps, ps)
        return sel
    ...
    # 1) real vs fake: dist(real, real) < dist(real, fake) + margin
...
def generate_sequence(model_path, priming_image_path, num_frames_to_generate, config: Config):
    ...
    with torch.no_grad():
        with torch.amp.autocast(device_type="cuda", dtype=torch.float16, enabled=(gen_config.DEVICE=="cuda")):
...
if __name__ == '__main__':
    ...
    model_file = os.path.join(cfg.MODEL_SAVE_PATH, "mtld_v2.0_checkpoint_epoch_40.pth")
    if os.path.exists(priming_image) and os.path.exists(model_file):
        generate_sequence(
            model_path="mtld_v2.0_checkpoint_epoch_40.pth",
